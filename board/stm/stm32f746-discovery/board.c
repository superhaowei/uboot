...
static const struct stm32f2_gpio_dsc ext_led_gpio[] = {
	//{STM32F2_GPIO_PORT_I, STM32F2_GPIO_PIN_11}
	//{STM32F2_GPIO_PORT_F, STM32F2_GPIO_PIN_15}
    {STM32F2_GPIO_PORT_A, STM32F2_GPIO_PIN_3}
    //{STM32F2_GPIO_PORT_A, STM32F2_GPIO_PIN_3}
    };
...
void led_config(int count)
{
   #define DELAY_COUNT (400*1000)
   stm32f2_gpio_config(&ext_led_gpio[0],STM32F2_GPIO_ROLE_GPOUT); 
   for(int i=0;i<count;i++){
        stm32f2_gpout_set(&ext_led_gpio[0],0);
        //udelay(DELAY_COUNT);
        my_delay(count);
        stm32f2_gpout_set(&ext_led_gpio[0],1);
        //udelay(DELAY_COUNT);
        my_delay(count);
   }
}

static const struct stm32f2_gpio_dsc psram_cre_gpio = {
    STM32F2_GPIO_PORT_G, STM32F2_GPIO_PIN_15
};

void psram_cre_reset(void)
{
    stm32f2_gpio_config(&psram_cre_gpio,STM32F2_GPIO_ROLE_GPOUT); 
    stm32f2_gpout_set(&psram_cre_gpio,0);

}
void psram_cre_set(void)
{
    stm32f2_gpio_config(&psram_cre_gpio,STM32F2_GPIO_ROLE_GPOUT); 
    stm32f2_gpout_set(&psram_cre_gpio,1);
}
...
int checkboard(void)
{
    ...
    printf("Board: STM32F769 Discovery Rev %s\n",
            CONFIG_SYS_BOARD_REV_STR);
